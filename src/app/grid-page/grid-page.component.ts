...
export class GridPageComponent implements OnInit {
  ...
  //orange icon
  likedIcon =
    'https://th.bing.com/th/id/R.59bb1857523a6ab9a363f0ffae12471e?rik=0%2fHvONgjAfHDWw&riu=http%3a%2f%2fwww.clker.com%2fcliparts%2f5%2fS%2ff%2fH%2fQ%2fT%2forange-thumb-like.svg.hi.png&ehk=kuWZO2rvkVoKZ92qQovZRzTb0GfLeVeaHZEWIS8EmUM%3d&risl=&pid=ImgRaw&r=0';
  notLikedIcon =
    'https://th.bing.com/th/id/OIP.KnvekyV7OUuJbWuk3j3xNQHaGn?w=1604&h=1432&rs=1&pid=ImgDetMain';
  ...
  fetchData() {
    const endpoint =
      'https://graphql.contentful.com/content/v1/spaces/40jcljdzym6w';
  ...
  filterDataBySubDomain(subDomain: string): void {
    if (subDomain === '') {
      ...
      this.filteredData1 = this.filteredData.filter(
        (item) => item.subDomain === subDomain
      ); // Filter data based on selected subdomain
  ...
  submitComment(contentId: string) {
    ...
    if (this.comment.trim() !== '') {
      if (this.userEmail !== null) {
        ...
        this.commentService
          .addComment(this.userEmail, this.comment, contentId)
          .subscribe(
            (response) => {
              console.log('Comment posted successfully:', response);
              // Handle the success response here and log the entered comment
              console.log(
                `Comment "${this.comment}" posted successfully. ContentId: ${contentId}, UserId: ${this.userEmail}`
              );
          ...
          if (response && response.hasOwnProperty('message')) {
            if (response.message === 'You have already liked this content') {
              ...
              if (isContentLiked) {
                ...
                this.likeCountMap.set(
                  contentId,
                  (this.likeCountMap.get(contentId) || 0) - 1
                ); // Decrease like count
                console.log(
                  'Like count after unlike:',
                  this.likeCountMap.get(contentId)
                ); // Log count after like // Log count after unlike
              ...
              console.log(
                `Content liked successfully. ContentId: ${contentId}, UserId: ${this.userEmail}`
              );
              ...
              console.log(
                'Like count after like:',
                this.likeCountMap.get(contentId)
              ); // Log count after like // Log count after like
            } else {
  ...
  get endIndex(): number {
    return Math.min(
      this.startIndex + this.commentsPerPage - 1,
  ...
  get paginatedComments(): any[] {
    ...
    const endIndex = Math.min(
      startIndex + this.commentsPerPage - 1,
      this.comments.length - 1
    );
